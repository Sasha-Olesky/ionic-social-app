(function () {
  ...
  function ionGallery($ionicPlatform, ionGalleryHelper, ionGalleryConfig) {
    ...
    function controller($scope) {
      ...
      var _fixedrowSize = $scope.ionFixedRowSize || false;

      var _drawGallery = function () {
		ionGalleryConfig.fixed_row_size = _fixedrowSize;
...
(function () {
  ...
  angular
    .module('ion-gallery')
    .directive('ionHomeGallery', ionHomeGallery);

  ionHomeGallery.$inject = ['$ionicPlatform', 'ionGalleryHelper', 'ionGalleryConfig'];

  function ionHomeGallery($ionicPlatform, ionGalleryHelper, ionGalleryConfig) {
    controller.$inject = ["$scope"];
    return {
      restrict: 'AE',
      scope: {
        ionHomeGalleryItems: '=ionHomeGalleryItems',
        ionGalleryRowSize: '=?ionGalleryRow',
        ionFixedRowSize: '=?ionFixedRow'
      },
      controller: controller,
    ...
    function controller($scope) {
      ...
      var _drawGallery = function () {
		  if($scope.ionHomeGalleryItems){
			ionGalleryConfig.fixed_row_size =_fixedrowSize;
			$scope.ionGalleryRowSize = ionGalleryHelper.getRowSize(_rowSize || ionGalleryConfig.row_size, $scope.ionHomeGalleryItems.length);
			$scope.items = ionGalleryHelper.buildGallery($scope.ionHomeGalleryItems, $scope.ionGalleryRowSize);
			$scope.responsiveGrid = parseInt((1 / $scope.ionGalleryRowSize) * 100);
		  }
      };
      ...
      (function () {
        $scope.$watch(function () {
          return $scope.ionHomeGalleryItems;//.length;
        }, function (newVal, oldVal) {
          if (newVal !== oldVal) {
            _drawGallery();
          }
		...
		$scope.likeTa = function(user){
			$scope.$parent.likeTa(user);
		};
		$scope.goPersonal = function(userid) {
			$scope.$parent.goPersonal(userid);
		};
    }
  }
})();
    ...
    function controller($scope){
      ...
      $scope.slideChanged = function(currentSlideIndex) {
        ...
        if(slidePosition === '0>1' || slidePosition === '1>2' || slidePosition === '2>0'){
		  ...
		  if (imageToLoad == 0) {
			  $scope.curIndex = $scope.ionGalleryItems.length;
		  } else {
			  $scope.curIndex = imageToLoad;
		  }
        }
		  ...
		  if (imageToLoad == $scope.ionGalleryItems.length-1) {
			  $scope.curIndex = 1;
		  } else {
			  $scope.curIndex = imageToLoad + 2;
		  }
        }
